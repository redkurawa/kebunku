...
export const ActivityProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
        ...
        if (!user) {
            ...
            unsubscribe = activityService.subscribeToUserActivities(
                        ...
                        try {
                            ...
                            const sortedData = [...data].sort((a, b) => {
                                const getMillis = (dateObj: any) => {
                                    if (dateObj && typeof dateObj.toMillis === 'function') return dateObj.toMillis();
                ...
                (err) => {
                    ...
                    if (isMounted) {
                        setError(err.message || 'Gagal mengambil data riwayat (Kemungkinan masalah Index Firestore).');
                        setLoading(false);
                    }
                }
            );
    ...
    const addActivity = async (activity: Omit<Activity, 'id' | 'createdAt' | 'userId'>) => {
        if (!user) {
    ...
    return (
        <ActivityContext.Provider value={{ activities, loading, error, addActivity, deleteActivity }}>
            {children}
