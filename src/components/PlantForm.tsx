...
const PlantForm: React.FC<PlantFormProps> = ({ onCancel, initialData }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="card" style={{ marginBottom: '1.5rem', border: initialData ? '2px solid var(--primary-color)' : '' }}>
            ...
            <button type="submit" className="btn btn-primary" style={{ width: '100%' }} disabled={loading}>
                ...
                {loading ? 'Menyimpan...' : (initialData ? 'Simpan Perubahan' : 'Tambah Tanaman')}
            </button>
        </form>
    );
};
