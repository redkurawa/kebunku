...
export const activityService = {
    ...
    subscribeToPlantActivities: (plantId: string, callback: (activities: Activity[]) => void, onError?: (error: Error) => void) => {
        const q = query(
    ...
    subscribeToUserActivities: (userId: string, callback: (activities: Activity[]) => void, onError?: (error: Error) => void) => {
        const q = query(
            collection(db, COLLECTION_NAME),
            where('userId', '==', userId)
        );
    ...
    uploadImage: async (userId: string, file: File, onProgress?: (percent: number) => void): Promise<string> => {
        ...
        const cloudName = 'dcporyxce';
        const uploadPreset = 'kebunku';
            ...
            return new Promise((resolve, reject) => {
                ...
                xhr.onload = () => {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        console.log('Service: Cloudinary upload success. URL:', response.secure_url);
                        resolve(response.secure_url);
                    } else {
                        const errorMsg = xhr.responseText || 'Gagal mengunggah foto ke Cloudinary';
