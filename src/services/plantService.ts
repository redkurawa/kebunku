...
// Helper function to normalize categoryId (take first word, capitalize first letter)
...
export const plantService = {
  ...
  addPlant: async (plant: Omit<Plant, 'id' | 'createdAt'>) => {
    try {
      const cleanedPlant = {
        ...
        categoryId: normalizeCategoryId(plant.categoryId),
  ...
  updatePlant: async (id: string, plant: Partial<Plant>) => {
    try {
      ...
      if (plant.categoryId !== undefined)
        cleanedPlant.categoryId = normalizeCategoryId(plant.categoryId);
      if (plant.variety !== undefined)
        cleanedPlant.variety = plant.variety.trim();
      if (plant.groupId !== undefined)
        cleanedPlant.groupId = plant.groupId.trim();
